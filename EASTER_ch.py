...
def train_model(target: str, model_name: str, model: Model, epochs):
    train_data = pd.read_csv(f'{BASE_DIR}/data_WWW2025/sentiment_acos/{target}_train.csv')
    train_data = DataGenerator(train_data['text'], train_data['sentiment'])
    valid_data = pd.read_csv(f'{BASE_DIR}/data_WWW2025/sentiment_acos/{target}_dev.csv')
    valid_data = DataGenerator(valid_data['text'], valid_data['sentiment'])
    callbacks = [
        ModelCheckpoint(f'{BASE_DIR}/data_WWW2025/pretrained/{model_name}.h5', 'val_accuracy', 1, True, True)
    ]
...
def load_model(model_name: str, kernel_size, filters, strides, units, include_text_cnn):
    ...
    model.load_weights(f'{BASE_DIR}/data_WWW2025/pretrained/{model_name}.h5')
    return model

def evaluate_model(target: str, model: Model):
    test_data = pd.read_csv(f'{BASE_DIR}/data_WWW2025/sentiment_acos/{target}_test.csv')
...
def analysis_sentiment(target_name,model_name,test_sentiment_fname,include_text_cnn):
    ...
    test_text_fname = f'{BASE_DIR}/data_WWW2025/acos/{target_name}_as_test.tsv'
...
if __name__ == "__main__":
    ...
    test_sentiment_fname = f'{BASE_DIR}/data_WWW2025/pred_senti/{target_name}_test_sentiment.csv'
